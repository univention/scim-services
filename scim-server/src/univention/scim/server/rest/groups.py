...
def set_group_service(service: Any) -> None:
    ...
    This function is called by the main application to inject the group service.

    Args:
        service: Group service implementation
    """
...
async def create_group(group: Group, response: Response) -> Group:
    ...
    try:
        created_group = await _group_service.create_group(group)
...
async def delete_group(group_id: str = Path(..., description="Group ID")) -> Response:
    ...
    try:
        success = await _group_service.delete_group(group_id)
        if not success:
